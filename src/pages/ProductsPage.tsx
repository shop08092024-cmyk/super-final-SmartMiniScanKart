...
const ProductsPage = ({ employeeView = false }: { employeeView?: boolean }) => {
  const { products, addProduct, updateProduct, deleteProduct, addToCart } = useStore();
  ...
  const [barcodeGenOpen, setBarcodeGenOpen] = useState(false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : (e && typeof e === "object" && "message" in e) ? String((e as any).message) : "Failed to save product";
  ...
  const handleAddStockToExisting = async () => {
    ...
    try {
      const finalStock = existingProductBarcode.stock + newStockInput;
      await updateProduct(existingProductBarcode.id, {
        stock: finalStock,
      });
      toast({
        title: "Stock updated ✓",
        description: `Added ${newStockInput} units to ${existingProductBarcode.name}. New stock: ${finalStock}`,
      });
      setExistingProductBarcode(null);
      setNewStockInput(0);
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : (e && typeof e === "object" && "message" in e) ? String((e as any).message) : "Failed to update stock";
      toast({
    ...
    <div className="page-container">
      <div className="mb-5 flex items-center justify-between animate-fade-in">
        ...
        {!employeeView && (
          <div className="flex gap-2">
            ...
            />
            <Button size="sm" variant="outline" className="rounded-xl gap-1.5" onClick={() => navigate("/sticker-print")}>
          ...
          <Card key={p.id} className="border-none shadow-soft transition-all duration-200 hover:shadow-medium animate-slide-up" style={{ animationDelay: `${i * 40}ms` }}>
            <CardContent className="flex items-center gap-3.5 p-4">
              ...
              <div className="flex flex-col gap-1.5">
                {employeeView ? (
                  <Button size="sm" className="rounded-lg" onClick={() => { addToCart(p, 1); toast({ title: "Added to cart" }); }}>
                    Add
                  </Button>
                ) : (
                  <>
                    <button onClick={() => navigate(`/sticker-print`)} className="rounded-lg p-2 text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground" title="Print sticker for this product">
                      <QrCode className="h-4 w-4" />
                    </button>
      ...
      {barcodeGenOpen && (
        <BarcodeGeneratorDialog
          open={barcodeGenOpen}
          onClose={() => setBarcodeGenOpen(false)}
        />
      )}
    </div>
  );
...
function BarcodeGeneratorDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const pagesNeeded = Math.ceil(quantity / STICKERS_PER_PAGE);

  const handleGenerate = () => {
    let finalCode = code.trim();
    if (!finalCode) {
      finalCode = "200" + Math.floor(1000000000 + Math.random() * 9000000000).toString().slice(0, 9);
      setCode(finalCode);
    }
    setReady(true);
  };
  ...
  const barcodeUrl = barcodeType === "qr"
    ? `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(code)}&bgcolor=ffffff&color=000000`
    : `https://bwipjs-api.metafloor.com/?bcid=code128&text=${encodeURIComponent(code)}&scale=2&rotate=N&includeText=true&backgroundcolor=ffffff`;

  const handlePrint = () => {
    if (!code) { toast({ title: "Generate a code first", variant: "destructive" }); return; }
    const win = window.open("", "_blank");
    if (!win) { toast({ title: "Popup blocked", description: "Please allow popups and try again", variant: "destructive" }); return; }

    const priceTag = price ? `<div class="sticker-price">₹${parseFloat(price).toFixed(2)}</div>` : "";
    const nameTag = name ? `<div class="sticker-name">${name}</div>` : "";
    const stickerHtml = Array(quantity).fill(null).map(() => `
      <div class="sticker">
        ${nameTag}
        <img src="${barcodeUrl}" alt="${code}" />
        <div class="sticker-code">${code}</div>
        ${priceTag}
      </div>
    `).join("");
...
<html><head>
  <meta charset="utf-8"/>
  <title>Sticker Sheet — ${quantity} stickers</title>
  <style>
    @page { size: A4; margin: 10mm 8mm; }
    * { box-sizing: border-box; }
    body { font-family: Arial, sans-serif; margin: 0; background: #fff; }
    .grid { display: grid; grid-template-columns: repeat(${STICKERS_PER_ROW}, 1fr); gap: 3mm; }
    .sticker { border: 1px dashed #bbb; border-radius: 4px; padding: 4mm 2mm 3mm; text-align: center; background: #fff; page-break-inside: avoid; display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 30mm; }
    .sticker-name { font-size: 9px; font-weight: bold; color: #111; margin-bottom: 2mm; max-width: 100%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .sticker img { max-width: 100%; max-height: 18mm; object-fit: contain; display: block; }
    .sticker-code { font-size: 7px; font-family: monospace; color: #444; margin-top: 1mm; }
    .sticker-price { font-size: 10px; font-weight: bold; color: #000; margin-top: 1mm; }
  </style>
</head><body>
      ...
      <DialogContent className="max-w-sm rounded-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <QrCode className="h-5 w-5 text-primary" /> Barcode / Label Builder
          </DialogTitle>
          <DialogDescription>Create and print sticker sheets for products</DialogDescription>
        </DialogHeader>
        <div className="space-y-3">
          <Input placeholder="Product name (optional)" value={name} onChange={e => setName(e.target.value)} className="h-10 rounded-xl" />
          <div className="flex gap-2">
            <Input placeholder="Barcode / code (blank = auto-generate)" value={code} onChange={e => { setCode(e.target.value); setReady(false); }} className="h-10 rounded-xl flex-1" />
            <Button onClick={handleGenerate} variant="secondary" className="h-10 rounded-xl text-xs shrink-0">
              {code && ready ? "Re-Gen" : "Generate"}
            </Button>
          </div>
          <Input placeholder="Price ₹ (optional)" type="number" value={price} onChange={e => setPrice(e.target.value)} className="h-10 rounded-xl" />

          {/* Quantity — always visible so user sets it before generating */}
          <div>
            ...
            <Input
              ...
              value={quantity}
              onChange={e => setQuantity(Math.max(1, Math.min(1000, +e.target.value)))}
              className="h-10 rounded-xl"
            />
            {quantity > 0 && (
              <p className="mt-1 text-xs text-muted-foreground">
                → <span className="font-semibold text-primary">{pagesNeeded} A4 page{pagesNeeded !== 1 ? "s" : ""}</span>
                <span className="text-muted-foreground"> ({STICKERS_PER_PAGE} stickers per page, {STICKERS_PER_ROW}×{ROWS_PER_PAGE} layout)</span>
              </p>
            )}
          </div>
          ...
          {ready && code && (
            <div className="space-y-3 pt-1">
              <div className="flex justify-center gap-2 p-1.5 bg-secondary/35 rounded-xl border border-border/40">
                <button onClick={() => setBarcodeType("code128")} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${barcodeType === "code128" ? "bg-primary text-white shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>Code-128 Barcode</button>
                <button onClick={() => setBarcodeType("qr")} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${barcodeType === "qr" ? "bg-primary text-white shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>QR Code</button>
              </div>
              <div className="flex flex-col items-center justify-center p-4 bg-white rounded-xl border border-border/45 min-h-[150px]">
                <img src={barcodeUrl} alt="Generated Barcode" className={barcodeType === "qr" ? "h-32 w-32" : "max-h-20 object-contain"} />
                <p className="mt-2 text-xs font-mono font-bold text-black">{code}</p>
                {name && <p className="text-xs text-slate-500 mt-0.5">{name}</p>}
                {price && <p className="text-sm font-bold text-slate-700 mt-0.5">₹{parseFloat(price).toFixed(2)}</p>}
              </div>

              {/* Print quantity summary */}
              <div className="rounded-xl bg-primary/6 border border-primary/15 px-3 py-2.5 flex items-center justify-between">
                <div>
                  <p className="text-xs font-bold text-foreground">{quantity} sticker{quantity !== 1 ? "s" : ""}</p>
                  <p className="text-[10px] text-muted-foreground">{pagesNeeded} A4 page{pagesNeeded !== 1 ? "s" : ""} · {STICKERS_PER_PAGE}/page</p>
                </div>
                <div className="grid gap-0.5" style={{ gridTemplateColumns: `repeat(${Math.min(STICKERS_PER_ROW, quantity)}, 8px)` }}>
                  {Array(Math.min(9, quantity)).fill(null).map((_, i) => (
                    <div key={i} className="h-2 w-2 rounded-sm bg-primary/40" />
                  ))}
                  {quantity > 9 && <span className="text-[7px] text-muted-foreground">+{quantity - 9}</span>}
                </div>
              </div>

              <div className="flex gap-2">
                <Button onClick={handleCopy} variant="outline" className="flex-1 rounded-xl h-11 text-xs gap-1.5">
                  Copy Code
                </Button>
                <Button onClick={handlePrint} className="flex-1 rounded-xl h-11 text-xs gradient-primary gap-1.5">
          ...
          {!ready && (
            <p className="text-center text-xs text-muted-foreground py-2">
              Set your options above, then click <strong>Generate</strong> to preview
            </p>
          )}
        </div>
      </DialogContent>
