...
import { ScanBarcode, Mail, Lock, Eye, EyeOff, Store, User, Phone, MapPin } from "lucide-react";
...
function ShopSetupModal({ open, onComplete }: { open: boolean; onComplete: () => void }) {
  const { saveProfile } = useShopProfile();
  const [form, setForm] = useState({ shopName: "", ownerName: "", phone: "", address: "", city: "", state: "", pincode: "" });
  ...
  const handleSave = async () => {
    ...
    try {
      await saveProfile({
        shopName: form.shopName, ownerName: form.ownerName, phone: form.phone,
        address: form.address, city: form.city, state: form.state, pincode: form.pincode,
        currency: "INR", invoicePrefix: "INV",
      });
      toast.success("Shop setup complete! Welcome to MiniScanKart 🎉");
      onComplete();
    } catch (e) {
    ...
    <Dialog open={open} onOpenChange={() => {}}>
      <DialogContent className="rounded-2xl max-w-sm mx-auto" onInteractOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          ...
          <DialogTitle className="text-center text-xl font-extrabold">Set Up Your Shop</DialogTitle>
          <DialogDescription className="text-center text-sm">Tell us about your business to get started</DialogDescription>
        </DialogHeader>
        <div className="space-y-3 mt-2">
          <div className="relative">
            <Store className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
            <Input placeholder="Shop / Store Name *" value={form.shopName} onChange={e => set("shopName", e.target.value)} className="pl-10 h-11 rounded-xl" />
          </div>
          <div className="relative">
            <User className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
            <Input placeholder="Owner Name *" value={form.ownerName} onChange={e => set("ownerName", e.target.value)} className="pl-10 h-11 rounded-xl" />
          </div>
          <div className="relative">
            <Phone className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
            <Input placeholder="Phone Number *" type="tel" value={form.phone} onChange={e => set("phone", e.target.value)} className="pl-10 h-11 rounded-xl" />
          </div>
          <div className="relative">
            <MapPin className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
            <textarea placeholder="Shop Address *" value={form.address} onChange={e => set("address", e.target.value)}
              className="w-full pl-10 pr-3 py-2.5 h-16 rounded-xl border border-input bg-background text-sm resize-none focus:outline-none focus:ring-2 focus:ring-ring" />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <Input placeholder="City" value={form.city} onChange={e => set("city", e.target.value)} className="h-10 rounded-xl text-sm" />
            <Input placeholder="State" value={form.state} onChange={e => set("state", e.target.value)} className="h-10 rounded-xl text-sm" />
          </div>
          <Input placeholder="Pincode" value={form.pincode} onChange={e => set("pincode", e.target.value)} className="h-10 rounded-xl text-sm" />
          <Button onClick={handleSave} disabled={saving} className="h-12 w-full rounded-xl gradient-primary text-sm font-semibold mt-2">
          ...
          <button onClick={onComplete} className="w-full text-center text-xs text-muted-foreground hover:text-foreground transition-colors py-1">
            Skip for now (you can set this up in Settings)
          </button>
        </div>
      </DialogContent>
    </Dialog>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isSignUp) {
      ...
      if (error) {
        ...
        toast.success("Account created! Check your email to confirm, then set up your shop.");
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-background px-6">
      ...
      {showShopSetup && (
        <ShopSetupModal
          open={showShopSetup}
          onComplete={() => { setShowShopSetup(false); navigate("/dashboard"); }}
        />
      )}
    </div>
  );
