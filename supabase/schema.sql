...
-- Employees / staff linked to the shop owner
...
create unique index if not exists employees_shop_email_idx on employees(shop_owner_id, lower(email));
create index if not exists employees_shop_owner_idx on employees(shop_owner_id);
...
drop policy if exists "Users read own payments" on payment_history;
drop policy if exists "Users insert own payments" on payment_history;
...
-- Add closing_balance to employees
alter table employees add column if not exists closing_balance numeric(10,2) not null default 0;
